...
export const characterNotesKeys = {
    ...
    detail: (characterId: string) => [...characterNotesKeys.all, characterId] as const,
  };

  export async function fetchCharacterNotes(token: string, characterId: string): Promise<CharacterQuickNotes> {
    return gameFetch<CharacterQuickNotes>(`/characters/${characterId}/notes`, token);
  }
