/**
 * Extrai texto do Jogador-2024.pdf (Cap. 4 — Espécies) via texto nativo ou OCR.
 * Padrão alinhado ao lotr-character-sheet/scripts/extract-pdf-ocr.mjs
 */
import fs from "fs";
...
/** Páginas do PDF (1-based) — Cap. 4 completo: antecedentes + espécies */
const START_PAGE = 184;
const END_PAGE = 211;
...
async function main() {
  ...
  const validation = {
    found: SPECIES_TERMS.filter((term) =>
      ocrLower.includes(term.toLowerCase()),
    ),
    missing: SPECIES_TERMS.filter(
      (term) => !ocrLower.includes(term.toLowerCase()),
    ),
  };

  fs.writeFileSync(path.join(OUT_DIR, "species-ocr.txt"), combined, "utf8");
  fs.writeFileSync(
    path.join(OUT_DIR, "species-ocr.json"),
    JSON.stringify(
      {
