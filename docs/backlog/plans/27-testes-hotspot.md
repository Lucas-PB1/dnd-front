...
- E2e (Next em `:3001`, API Nest, Supabase): credenciais em `.env.local` — `CYPRESS_LOGIN_EMAIL` / `CYPRESS_LOGIN_PASSWORD` ou `NEXT_PUBLIC_DEV_LOGIN_EMAIL` / `NEXT_PUBLIC_DEV_LOGIN_PASSWORD`, mais `NEXT_PUBLIC_SUPABASE_*`.
- Um spec por classe PHB em `cypress/e2e/sheet/classes/`. Progressão: `npm run test:e2e:sheet:level-up`, `…:subclass`, `…:feat`.
